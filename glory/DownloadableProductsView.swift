//
//  DownloadableProductsView.swift
//  Chnen
//
//  Created by Surbhi Handa on 10/16/17.
...
class DownloadableProductsView: UIViewController,UIScrollViewDelegate,UIWebViewDelegate,UITextViewDelegate,UITextFieldDelegate
{
    ...
    @IBOutlet weak var DownloadableView: UIView!
    @IBOutlet weak var downloadLinks: UIView!
    @IBOutlet weak var ConstraintDownloadableViewHeight: NSLayoutConstraint!
    ...
    var dictLinks = [String : Any]()
    var arrOptions = [String: Any]()
    var arrrows = [[String : Any]]()
    var arrCheckbox = [[String: String]]()
    var arrDownLoadCheckbox = [[String: Any]]()
    ...
    func ProductdetailResponse(obj: Product)
    {
        ...
        // MARK:- Downloadable Options
            
            dictLinks = obj.links
            arrrows = obj.rows
            
            downloadLinks = CustomOptionUI.sharedInstance().checkboxDownloadable(dict: dictLinks , view: downloadLinks ,target: self ,selector : #selector(self.MultipleDownloadMethod))
            
            ConstraintDownloadableViewHeight.constant = downloadLinks.frame.size.height+downloadLinks.frame.origin.y
    ...
    @IBAction func AddToBagButton(_ sender: AnyObject)
    {
        if defaults.bool(forKey: dis_login)
        {
            ...
            for i in 0..<arrDownLoadCheckbox.count {
                parameters["links[\(i)]"] = arrDownLoadCheckbox[i]["id"]
                print(i)
            }
            print(parameters)
            let objProduct = Product()
    ...
    //Mark:- multiple download
    func MultipleDownloadMethod(sender: UIButton)
    {
        _ = [[String: String]]()
        for dict in arrrows
        {
            if Int((dict[kid] as? String)!) == sender.tag
            {
                if sender.currentImage == #imageLiteral(resourceName: "Uncheck") {
                    sender.setImage(#imageLiteral(resourceName: "Check"), for: .normal)
                    arrDownLoadCheckbox.append(dict)
                    self.arrOptions[sender.accessibilityLabel!] = arrDownLoadCheckbox as [[String : Any]]
                } else
                {
                    sender.setImage(#imageLiteral(resourceName: "Uncheck"), for: .normal)
                    if  arrDownLoadCheckbox.contains(where: { (dict) -> Bool in
                        arrDownLoadCheckbox.remove(at: arrDownLoadCheckbox.index(where: { (dict) -> Bool in
                            return true })!)
                        self.arrOptions[sender.accessibilityLabel!] = arrDownLoadCheckbox
                        return true})
                    {
                    }
                }
    ...
    func calculateDownloadPrice ()
    {
        ...
        for i in arrOptions
        {
            if let d : [String: Any] = i.value as? [String: Any]
            {
                if (d["link_price"] as? String) != "0.00"
                {
                    total =  total + Float(d["link_price"] as! String )!
                }
            }
            else
            {
                for j in i.value as! [[String: Any]]
                {
                    if (j["link_price"] as? String) != "0.00" {
                        total =   total + Float(((j["link_price"] as? String)?.replacingOccurrences(of: ",", with: ""))!)!
                    }
                }
            }
        }
