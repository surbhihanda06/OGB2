//  ConfigurableProductsView.swift
//  Chnen
//
//  Created by Navjot_sharma on 12/27/16.
...
class ConfigurableProductsView: UIViewController  ,UIScrollViewDelegate,UIWebViewDelegate,UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout,UITextViewDelegate,UITextFieldDelegate
{
    ...
    @IBOutlet weak var downPopUp: UIView!
    ...
    var dictSize = [String : Any]()
    var arrSizeOptions = [[String: Any]]()
    var strSize = String()
    var dictColor = [String : Any]()
    var arrColorOptions = [[String : Any]]()
    var strColor = String()
    var mediaUrl = String()
    ...
    func deviceDidRotate(notification: NSNotification) {
        ...
        if self.viewCustomOptions != nil {
            for subview in viewCustomOptions.subviews   {
                subview.frame.size.width = viewCustomOptions.frame.size.width
            }
        }
    }
    ...
    func ProductdetailApi()
    {
        ...
        let parameters: Parameters = [ksalt: SALT as AnyObject , PROD_ID : strProd_id as AnyObject, VISITOR_ID : globalStrings.Visitor_id as AnyObject, CUST_ID : globalStrings.Cust_id as AnyObject  , kcstore: one as AnyObject]
        let objProduct = Product()
        objProduct.ProductDetailApi(dict: parameters as [String : AnyObject], target: self, selector: #selector(self.ProductdetailResponse))
    ...
    func ProductdetailResponse(obj:Product)
    {
        if obj.resultText == ksuccess
        {
            ...
            arrAttributes = obj.attributes
            mediaUrl =  obj.media_url
            if arrAttributes.count != 0
            {
                dictColor = arrAttributes[0]
                print(dictColor)
                arrColorOptions = dictColor[koptions] as! [[String : Any]]
                if arrColorOptions.count != 0
                {
                    selectedColor = 0
                    strColor =  arrColorOptions[0][kid] as! String
                    ConstraintColorViewHeight.constant = 69
                    colorView.isHidden = false
                    visualization_type =  dictColor[kvisualization_type] as! String
                    ColorCollectionview.reloadData()
                }
                else
                {
                    ConstraintColorViewHeight.constant = 0
                    ConstraintDetailViewY.constant = -69
                    colorView.isHidden = true
                }
                dictSize = (arrColorOptions[0][khave_stock] as! [AnyObject])[0] as! [String : AnyObject]
                arrSizeOptions = dictSize[kavailable] as! [[String: Any]]
                print(dictSize)
                if arrSizeOptions.count != 0 {
                    selectedSize = arrSizeOptions.count
                    btnSize.isHidden = false
                } else {
                    btnSize.isHidden = true
                }
            }
            else {
                ConstraintColorViewHeight.constant = 0
    ...
    // MARK:-  SizeButton method
    
    @IBAction func SizeButton(_ sender: UIButton)
    {
        UIView.animate(withDuration: 1.0, animations: {
            self.downPopUp.isHidden = false
        })
    ...
    @IBAction func WishlistButton(_ sender: UIButton)
    {
        ...
        {
            globalStrings.showALert(message: NotLoggedInWishlist, target: self)
        }
    }
    ...
    {
        return collectionView == SizeCollectionView ? arrSizeOptions.count : arrColorOptions.count
    }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell
    {
        ...
        {
            let cell = collectionView.dequeueReusableCell(withReuseIdentifier: cSizeCollectionCell, for: indexPath as IndexPath) as! SizeCollectionCell
            cell.lblSize.text = arrSizeOptions[indexPath.row][klabel] as? String
            if selectedSize == indexPath.row
            {
                cell.layer.borderColor = buttonColor.cgColor
            }
            else
            {
        ...
        {
            let cell = collectionView.dequeueReusableCell(withReuseIdentifier: cColorCollectionCell, for: indexPath as IndexPath) as! ColorCollectionCell
            ...
            {
                let imgUrlStr = (mediaUrl + "icons/o_" + (arrColorOptions[indexPath.row][kicon] as? String)!)
                let urlImgg = URL(string: imgUrlStr)
                cell.imgColor .sd_setImage(with: urlImgg)
            }
            else if visualization_type == "1"
            {
                print(arrColorOptions[indexPath.row][kcolor] as! String!)
                var str : String = arrColorOptions[indexPath.row][kcolor] as! String
                str = str.replacingOccurrences(of: "#", with: "")
                let num2 = Int(str, radix: 16)
                print(num2!)
                if let num2 = Int(str, radix: 16) {
                    cell.imgColor.backgroundColor = UIColor.init(netHex: num2 )
                }
            }
            else
            {
                cell.imgColor.image = UIImage(named: (arrColorOptions[indexPath.row][kid] as? String)!)
            }
            return cell
        }
    }
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, insetForSectionAt section: Int) -> UIEdgeInsets {
        ...
        {
            let totalCellWidth = 30 * arrColorOptions.count
            let totalSpacingWidth = 10 * (arrColorOptions.count - 1)
            
            let leftInset = (ColorCollectionview.frame.size.width - CGFloat(totalCellWidth + totalSpacingWidth)) / 2;
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath)
    {
        if collectionView == SizeCollectionView
        {
            print(arrSizeOptions[indexPath.row][kid] as! String )
            selectedSize = indexPath.row
            strSize = arrSizeOptions[indexPath.row][kid] as! String
            btnSize.setTitle("SIZE: \(arrSizeOptions[indexPath.row][klabel] as! String)", for: .normal)
            UIView.animate(withDuration: 2.0, animations: {
                self.downPopUp.isHidden = true
            })
            SizeCollectionView.reloadData()
        }
        else
        {
            ...
            strColor =  arrColorOptions[indexPath.row][kid] as! String
            strSize = ""
            selectedSize = arrSizeOptions.count
    ...
    func alertView(View: UIAlertController!, clickedButtonAtIndex buttonIndex: Int)
    {
        if view.tag == 9
        {
    ...
    @IBAction func AddToBagButton(_ sender: AnyObject)
    {
        if defaults.bool(forKey: dis_login)
        {
            ...
            if dictSize.count != 0  {
                parameters["super_attribute[" + (dictSize[kid] as! String) + "]"] = strSize
            }
            if dictColor.count != 0  {
                parameters["super_attribute[" + (dictColor[kid] as! String) + "]"] = strColor
            }
            for i in arrOptions
            {
    ...
    func calculatePrice ()
    {
        var total = strPrice == "" ? 0 : Int(strPrice)
        for i in arrOptions
        {
            if let d : [String: Any] = i.value as? [String: Any]
            {
                if (d["price"] as? String) != "0.00" {
                    total =  total! + Int(d["price"] as! String )!
                }
            }
            else
            {
                for j in i.value as! [[String: Any]]
                {
                    if (j["price"] as? String) != "0.00" {
                        total =   total! + Int(j["price"] as! String )!
                    }
                }
            }
        }
